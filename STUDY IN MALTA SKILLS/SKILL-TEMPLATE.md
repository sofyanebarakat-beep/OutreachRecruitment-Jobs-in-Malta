...
For every new root-level article, create both deployment targets:
...
The two files must contain the same article HTML. This prevents clean URL 404s on hosts that do not reliably rewrite `/[slug]` to `/[slug].html`.
